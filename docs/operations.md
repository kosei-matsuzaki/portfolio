...
詳細ページの「最終更新」は**手で書きません**。`npm run updated`
（`scripts/gen-updated.mjs`）が git 履歴から出して `src/data/updatedAt.ts` を
生成します。`npm run build` の前に自動で走るので、公開されるものは常に最新です。

見ているのは 2 つで、新しいほうを採ります。

- `src/data/projects.ts` の**その作品のエントリだけ**（`git log -L` で行範囲を
  履歴に沿って追う。ほかの作品を直しても日付は動かない）
- その作品が参照している素材（`public/images/…` `public/video/…`）

まだコミットしていない変更がその作品に掛かっていれば**今日**になります。
「直す → ビルド → コミット」で日付が 1 コミットぶん遅れないようにするためです。

元リポジトリの更新日ではなく、**このサイトを直した日**であることに注意してください
（元リポジトリ側の進みは `npm run check` が見ます）。

> CI は `fetch-depth: 0` で clone します。浅い clone だと履歴が無く日付を出せません。
> git を読めない環境では書き換えずに終わるので、コミット済みの値がそのまま使われます。
